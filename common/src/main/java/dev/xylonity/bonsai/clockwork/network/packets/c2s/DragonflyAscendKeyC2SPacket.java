...
public record DragonflyAscendKeyC2SPacket(boolean ascending) {
    ...
    private static final ResourceLocation ID = Clockwork.resource("dragonfly_ascend_key");
    ...
    public static final ServerboundPacketType<DragonflyAscendKeyC2SPacket> TYPE =
            PacketType.serverbound(
                    ...
                    (msg, player) -> {
                        if (player.getVehicle() instanceof DragonflyEntity dragonflyEntity && dragonflyEntity.getControllingPassenger() == player) {
                            if (dragonflyEntity.getState() != 1) {
                                dragonflyEntity.setState(1);
                            }

                            dragonflyEntity.setAscending(msg.ascending);
                        }
